    ...
    if not df_gastos.empty:
        ...
        if 'MCNCUENTA' in df_gastos.columns and 'MCNVALDEBI' in df_gastos.columns:
            ...
            for cat, valor in agrupado.items():
                if valor > 0: dict_gastos[cat] = float(valor)
    return dict_gastos
...
def armar_resumen_gerencial(df_global: pl.DataFrame, df_detallado: pl.DataFrame, ajustes: dict = None) -> pd.DataFrame:
    ...
    if "Centro_Costos" in df_global.columns:
        df_global = df_global.with_columns(
            pl.col("Centro_Costos").cast(pl.Utf8).str.extract(r"(\d{5})").replace_strict(mapeo_cajas_bd, default=pl.col("Centro_Costos")).alias("Centro_Costos")
        )

    finansuenos_caja = df_global.filter(
        (pl.col("Origen") == "CAJA") & 
        (pl.col("Tercero").fill_null("").str.to_uppercase().str.contains("FINANSUENOS")) &
        (~pl.col("Documento_Referencia").fill_null("").str.to_uppercase().str.starts_with("ES"))
    )["Egreso"].sum()
    if finansuenos_caja > 0: dict_gastos_oficial["Finansuenos Sas"] = dict_gastos_oficial.get("Finansuenos Sas", 0) + finansuenos_caja

    libranzas_bancos = df_global.filter((pl.col("Origen") == "CAJA_BANCOS") & (pl.col("Categoria_Flujo") == "Libranzas"))["Egreso"].sum()
    if libranzas_bancos > 0: dict_gastos_oficial["Libranzas"] = dict_gastos_oficial.get("Libranzas", 0) + libranzas_bancos

    aportes_bancos = df_global.filter((pl.col("Origen") == "CAJA_BANCOS") & (pl.col("Categoria_Flujo") == "Seguridad Social"))["Egreso"].sum()
    if aportes_bancos > 0: dict_gastos_oficial["Seguridad Social"] = dict_gastos_oficial.get("Seguridad Social", 0) + aportes_bancos
    ...
    # LA MATEMÁTICA PURA (Sin restas dobles, porque la calculadora ya limpió las anuladas)
    ingresos_bancos = total_bancos_ingresos - total_bancos_salidas_traslados + salida_traslado_alianza
    ingresos_otros_bancos = df_bancos.filter(pl.col("Origen") != "ALIANZA")["Ingresos_Operativos"].sum()
    ingreso_real_alianza = ingresos_bancos - ingresos_otros_bancos

    ingresos_caja_bruto = df_global.filter(
        (pl.col("Origen") == "CAJA") & 
        (pl.col("Ingreso") != 0) & 
        (pl.col("Categoria_Flujo") != "Traslado_Salida") &
        (~pl.col("Concepto").fill_null("").str.to_uppercase().str.contains("ANULA"))
    )["Ingreso"].sum()
    ...
    for row in df_bancos.iter_rows(named=True):
        ...
        valor_neto = ingreso_real_alianza if row["Origen"] == "ALIANZA" else row["Ingresos_Operativos"]
    ...
    df_caja_ingresos_ccosto = df_global.filter((pl.col("Origen") == "CAJA") & (pl.col("Ingreso") != 0) & (pl.col("Categoria_Flujo") != "Traslado_Salida") & (~pl.col("Concepto").fill_null("").str.to_uppercase().str.contains("ANULA"))).group_by("Centro_Costos").agg(pl.col("Ingreso").sum().alias("Valor")).sort("Valor", descending=True)
    desglose_caja_ingresos = [{"Concepto": f"   > C.C: {safe_title(row['Centro_Costos'])}", "Valor": row["Valor"]} for row in df_caja_ingresos_ccosto.iter_rows(named=True)]
    ...
    df_caja_salidas_ccosto = df_global.filter((pl.col("Origen") == "CAJA") & (pl.col("Egreso") != 0) & (pl.col("Categoria_Flujo") == "Operacion_Normal")).group_by("Centro_Costos").agg(pl.col("Egreso").sum().alias("Valor")).sort("Valor", descending=True)
    desglose_caja_salidas = []
    for row in df_caja_salidas_ccosto.iter_rows(named=True):
        nombre_caja = safe_title(row['Centro_Costos'])
    ...
    desglose_proveedores_caja = [{"Concepto": f"   > Prov Caja: {safe_title(row['Tercero'])}", "Valor": row["Valor"]} for row in df_prov_pagos_caja.group_by("Tercero").agg(pl.col("Egreso").sum().alias("Valor")).sort("Valor", descending=True).iter_rows(named=True)]

    df_prov_pagos_bancos = df_global.filter((pl.col("Origen") == "CAJA_BANCOS") & (pl.col("Egreso") != 0) & (pl.col("Codigo_Proveedor").fill_null("").is_in(codigos_proveedores)))
    total_pagos_bancos = df_prov_pagos_bancos["Egreso"].sum()
    desglose_proveedores_bancos = [{"Concepto": f"   > Prov Banco: {safe_title(row['Tercero'])}", "Valor": row["Valor"]} for row in df_prov_pagos_bancos.group_by("Tercero").agg(pl.col("Egreso").sum().alias("Valor")).sort("Valor", descending=True).iter_rows(named=True)]
